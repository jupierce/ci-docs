...
In order to make sure the job is reporting on the build watcher dashboards so build wathcers are alerted if it starts failing, rename the job to begin with release- and not canary-, choosing the final name of the suite. This name change will include your job in the release job metrics and dashboards.
